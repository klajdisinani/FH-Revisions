...
%Simulation, design, and control of dynamical systems play an important role in numerous scientific and industrial tasks such as signal propagation in the nervous system\cite{kellems2009low}, heat dissipation, prediction of major weather events \cite{BoyP11,Sha03,Ant05}. 
Simulation, design, and control of dynamical systems play an important role in numerous scientific and industrial tasks such as \textcolor{blue}{signal propagation in the nervous system\cite{kellems2009low};  the synthesis  of
...
\end{equation}
\noindent
where %$  \x(0) = \mathbf{0}, \A \in \R^{n\times n}$,
$ \A \in \R^{n\times n}$,
...
\end{aligned}
\end{equation}
where as in \eqref{fom1}, %$\x_r(0) = \mathbf{0}, \h_r(t)=\C_r e^{\A_r t}\B_r$
...
%In this paper,  we explore $\ch_2(\tf)$ optimality for model reduction over a finite time horizon. 
...
\textcolor{blue}{In this paper,  we explore optimal model reduction over a finite time horizon. We use a time-limited version of the $\ch_2$ norm, denoted by $\ch_2(\tf)$, to quantify the model reduction error.}
...
\textcolor{blue}{The advantage of the interpolation framework we will develop  is that 
...
\end{equation}
\end{theorem}
{\color{blue} where $\HH'(s)$ denotes the derivative of $\HH(s)$ with respect to $s$.}
{\color{blue}For more details on Theorem \ref{h2thm}, see {\cite {GugBA08,AntBG10b}}.}
...
%Also, in case of an unstable system, since the impulse grows unbounded as $t \to \infty$, the regular $\ch_2$ measure cannot be applied. Therefore, we will focus on model reduction on a finite time horizon. 
...
$[0,t_f]$, the $\ch_2(\tf)$\footnote{\textcolor{blue}{Even though we are using an error measure specifically formulated in the time domain,   following \cite{RedK17,GoyR17} we choose to use  the $\ch_2(\tf)$ notation, despite the term $\ch_2$  is mostly associated with a measure defined in the frequency domain. Our main reason is to keep the connection to the infinite horizon problem where the measure in the frequency- and time-domains are equivalent. And more importantly, as in the regular $\ch_2$ case, the optimality conditions will still appear as interpolation conditions in the frequency domain. 
 %In the Since the time domain and frequency domain functions are equivalent,  this notation is appropriate. Furthermore, since the infinite horizon, and finite horizon case appear to be connected, the notation $\mathcal{H}_2(t_f)$ illustrates this connection. \cite{RedK17,GoyR17} use similar notation.
 }}
...
{\color{blue} We note that the interval of interest is problem dependent and the choice of the interval, i.e., the choice of $t_f$, may depend on the model. However, these optimality conditions hold for any choice of $t_f>0$.}

%Remark
\begin{remark}
{\color{blue} In the infinite-horizon case, if  $\HH_r(s)$ is the best $\ch_2$ approximation to $\HH(s)$, then $\HH_r(s)$ interpolates $\HH(s)$. However, in the finite-horizon case, 
the interpolant is $\G_r(s)$, and the interpolated function is $\G(s)$; thus $\HH_r(s)$ does not interpolate $\HH(s)$. To give more intuition about these resulting interpolation conditions, considere the time-limited function $\g(t)$ such that 
$\g(t)=\h(t)$ when   $t\leq {\tf}$ and $\g(t)=0$ when when $t > {\tf}$.
%\begin{align*}
%\g(t)=
% \begin{cases} 
%      \h(t) &    t\leq {\tf}, \\    
...
%   \end{cases}
%\end{align*}
A direct calculation shows that $\G(s)$ is the Laplace transformation of $\g(t)$. Similarly let $\g_r(t)$ 
denote the time-limited version of $\h_r(t)$. Then its Laplace transformation is 
 $\G_r(s)$.  Therefore, the optimality conditions  \eqref{eqn:cond1}--\eqref{eqn:cond3}
 correspond to optimal interpolation of $\G(s)$ (Laplace transform of a time-limited function)
 by $\G_r(s)$, (Laplace transform of another time limited function.) The fact that  
...
\textcolor{blue}{Finally, in Figure \ref{fig:impulse}, we compare the error in the impulse responses due to POD and \FH \ for the ISS model. For both methods, POD and \FH\  the reduced model was of order $r=14$. As we can see from the graph, \FH \ clearly outperforms POD on the time interval $[0, 1]$.}

 
 \begin{figure}[H]
 \centering
   \includegraphics [scale=0.35]{ISS1RErrorSmooth}
      \caption{\FH \ and POD for the ISS 1R model \label{fig:impulse}}
 \end{figure}
